...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        # self.buffer = Buffer(num_inputs, action_space.shape[0],self.buffer_size, args.seed)
        ...
        if self.policy_type == "Gaussian":
            ...
            self.policy = Squashed_Gaussian_Actor(num_inputs, action_space.shape[0]).to(self.device)
    ...
    def update_parameters(self, batch_size, updates):
        ...
        # state_batch, action_batch, reward_batch, next_state_batch, mask_batch = self.buffer.get_batch(batch_size=batch_size)
        ...
        with torch.no_grad():
            next_state_action, next_state_log_pi = self.policy(next_state_batch)
        ...
        #나여깄어~~~
        A1,A2 = self.critic.get_attention(state_batch, action_batch)
        A1 = torch.sum(A1, dim=0, keepdim=True)
        A2 = torch.sum(A2, dim=0, keepdim=True)
        AA = A1+A2
        AAA = torch.argmax(AA)


        return qf1_loss.item(), qf2_loss.item(), policy_loss.item(), alpha_loss.item(), alpha_tlogs.item(),AAA
