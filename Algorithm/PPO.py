...
from Model.Model import Policy_net, V_net
from Model.Model import Gaussian_Actor
...
class PPO:
    def __init__(self, state_dim, action_dim, device, args):
        ...
        if self.discrete == True:
            self.actor = Policy_net(self.state_dim, self.action_dim).to(self.device)
        else:
            self.actor = Gaussian_Actor(self.state_dim, self.action_dim).to(self.device)

        self.critic = V_net(self.state_dim).to(self.device)
    ...
    def train_actor(self, batch_s, batch_a, batch_old_log_policy, batch_advantages,c2=0.01):
        ...
        if self.discrete == True:
            ...
            log_policy = dist.log_prob(batch_a).sum(1,keepdim=True)
            ratio = (log_policy - batch_old_log_policy).exp()
            surrogate = ratio * batch_advantages
        if self.ppo_mode == 'clip':
            clipped_surrogate = torch.clamp(ratio, 1 - self.clip, 1 + self.clip) * batch_advantages
            # actor_loss = -torch.minimum(surrogate, clipped_surrogate)-c2*dist.entropy ()#Adding Entropy term
    ...
    def train(self, training_num):
        ...
        # print("test----------------")
        # pi = self.actor.dist(s)
        # od_log_prob = pi.log_prob(a)
        ...
        for i in range(training_num):
            for epoch in range(training_num2):
                ...
                batch_old_log_policy = log_prob[batch_index].sum(1,keepdim=True)
                # print(batch_old_log_policy)
                # print("od==============")
                # print(od_log_prob)
