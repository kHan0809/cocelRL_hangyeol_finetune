...
parser.add_argument('--alpha', type=float, default=0.2, metavar='G',help='Temperature parameter α determines the relative importance of the entropy\term against the reward (default: 0.2)')
...
for iter in range(1,6):
    # log
    if args.log == True:
        f = open("./1log" + str(iter) + "SAC_QA" + ".txt", 'w')
        f.close()
    ...
    for i_episode in itertools.count(1):
        ...
        while not done:
            if args.start_steps > total_numsteps:
                ...
                if episode_steps != 0:
                    action[AAA] +=  0.1*np.random.normal(0,1.0,1)
            ...
            if len(agent.buffer) > args.batch_size:
                ...
                for i in range(args.updates_per_step):
                    ...
                    critic_1_loss, critic_2_loss, policy_loss, ent_loss, alpha, AAA = agent.update_parameters(args.batch_size, updates)
            ...
            # agent.buffer.store(state, action, reward, next_state, mask)  # Append transition to memory
        ...
        if i_episode % 10 == 0 and args.eval is True:
            ...
            f = open("./1log" + str(iter) + "SAC_QA" + ".txt", 'a')
